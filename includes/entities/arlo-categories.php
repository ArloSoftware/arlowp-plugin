...
class Categories {
	...
	static function get_flattened_category_list_for_filter($base_category, $exclude_category, $import_id) {
		$cache_key = md5('get_flattened_category_list_for_filter' . serialize(func_get_args()));
		
		return CacheControl::get_cached_object(
			$cache_key,
			function() use ($base_category, $exclude_category, $import_id) {
	...
	static function get_merged_tree($categories, $import_id) {
		$cache_key = md5('get_merged_tree' . serialize(func_get_args()));
                            
		return CacheControl::get_cached_object(
			$cache_key,
			function() use ($categories, $import_id) {

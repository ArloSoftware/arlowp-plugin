...
class Arlo_For_Wordpress_Events extends Arlo_For_Wordpress_Lists  {
	...
	function column_e_code($item) {
		$actions = array(
            'edit' => sprintf('<a href="https://%s.arlo.co/management/Courses/ScheduleItem.aspx?id=%d" target="_blank">Edit</a>', esc_url($this->platform_name), $item->e_arlo_id),
	...
	public function get_new_link() {
		return sprintf('https://%s.arlo.co/management/Console/#/events/new/', esc_url($this->platform_name) );
	}
	
	public function get_list_link() {
		return sprintf('https://%s.arlo.co/management/Courses/Courses2.aspx', esc_url($this->platform_name) );
	}			
}

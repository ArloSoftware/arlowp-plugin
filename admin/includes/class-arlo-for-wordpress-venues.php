...
class Arlo_For_Wordpress_Venues extends Arlo_For_Wordpress_Lists  {
	...
	public function column_v_name($item) {
		$actions = array(
            'edit' => sprintf('<a href="https://%s.arlo.co/management/Venues/Venue.aspx?id=%d" target="_blank">Edit</a>', esc_url($this->platform_name), $item->v_arlo_id),
	...
	public function get_new_link() {
		return sprintf('https://%s.arlo.co/management/Venues/Venue.aspx?i=1', esc_url($this->platform_name) );
	}
	
	public function get_list_link() {
		return sprintf('https://%s.arlo.co/management/Venues/Venues.aspx', esc_url($this->platform_name) );
	}
